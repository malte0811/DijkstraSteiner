...
private:
    ...
    LabelMap<Cost, false> _best_cost_bounds;
    std::vector<std::unordered_map<TerminalSubset, Cost>> _fixed_map;
...
Cost DijkstraSteiner<FC>::get_optimum_cost() {
    ...
    while (not _heap.empty()) {
        ...
        if (_fixed_map.at(next_label.first.global_index).count(next_label.second)) {
            continue;
        }
        auto const cost_here = _best_cost_bounds.get(next_label);
        _fixed_map.at(next_label.first.global_index).insert({next_label.second, cost_here});
...
void DijkstraSteiner<FC>::for_each_disjoint_fixed_sink_set(Label const& base_label, Consumer const out) const {
    ...
    if (num_subsets <= num_fixed_sets) {
        ...
        auto const& fixed_map = _fixed_map.at(base_label.first.global_index);
        do {
            ...
            if (current_set.any()) {
                auto const it = fixed_map.find(current_set);
                if (it != fixed_map.end()) {
                    out(current_set, it->second);
                }
            }
